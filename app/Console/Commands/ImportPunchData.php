...
class ImportPunchData extends Command
{
     ...
     * @var string
     */
    protected $signature = 'punches:import {date?}';
    ...
    public function handle()
    {
        ...
        {
            $date = Carbon::parse($this->argument('date').'-'.date('Y'))->toDateString();
            DB::table(caseMatchTable('DeviceLogs_Processed'))
                            ->whereDate('LogDate', $date)->orderBy('DeviceLogId')
        ...
        else
        {
            ...
            if($lastRecord)
            {
                ...
                DB::table(caseMatchTable('DeviceLogs_Processed'))->where('DeviceLogId', '>=', $lastLogId)
                                ->orderBy('DeviceLogId')->chunk(500, function($datas){
                                    $this->importPunches($datas);
                                });
            }
            else
    ...
    private function importPunches($datas)
    {
        foreach($datas as $data)
        {
            ...
            $punch = Punch::where([ 'emp_code'=> $data->UserId, 'punch_date'=> $punchDate ])->first();
